...
kObservations <- function(rows) {
    ...
    s1 = Reduce("+", Map(function(data) data$SSD, dataList)) / Reduce("+", Map(function(data) data$f, dataList));
    Ba = calcBa(k, fs, f1, s1, dataList)
    html("<h2>Tester hypotese om varianshomogenitet</h2>")
    eq("H_{0\\sigma^2}: \\sigma_1^2 = \\dots = \\sigma_k^2 = \\sigma^2")
    eq(int("C = ... = `calcC(k, fs, f1)`"))
